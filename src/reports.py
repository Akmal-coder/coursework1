...
def _default_name(func: Callable) -> str:
...
def report_writer(arg: Optional[Any] = None) -> Callable:
    """
    ...
    def decorator(func: Callable) -> Callable:  # @report_writer("file.json") или @report_writer()
        @wraps(func)
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            result = func(*args, **kwargs)
...
def _write_json(filename: str, result: Any) -> None:
    """
