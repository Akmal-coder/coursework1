...
def load_json_data(file_path: str) -> dict:
    """
    Возвращает данные о финансовых транзакциях из JSON
    """
    try:
        if not os.path.exists(file_path):
            logger.warning("File not found")
...
def get_card_infos(transactions: list[dict]) -> list[dict]:
    """
    Возвращает информацию о картах
    """
    if not transactions:
        return []
...
def get_current_exchange_rate(currency_codes: list) -> list:
    """
    Функция возврата текущего курса валют
    """
...
def get_stock(stocks: list) -> list:
    """
    Функция возврата текущего курса акций
    """
